// Este archivo usa ES Modules (extensión .mjs)
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

// En ES Modules, __dirname no está disponible directamente
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

console.log('===== NORMALIZACIÓN DE DB (ES Modules) =====');
console.log(`Directorio actual: ${__dirname}`);

// Ruta al archivo de la base de datos
const dbPath = path.join(__dirname, '..', 'src', 'lib', 'data', 'db.json');
const outputPath = path.join(__dirname, '..', 'src', 'lib', 'data', 'db-normalized-esm.json');
